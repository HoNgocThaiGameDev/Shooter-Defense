...
public class Student : IComparable
{
    ...
    public int CompareTo(object o_other)
    {
        if (o_other == null)
            return 1;
        Student sturent_b = o_other as Student;
        if (this.name_student.Length > sturent_b.name_student.Length)
        {
            return 1;
        }
        else if (this.name_student.Length < sturent_b.name_student.Length)
        {
            return -1;
        }
        else
        {
            if (this.code > sturent_b.code)
            {
                return 1;
            }
            else if (this.code < sturent_b.code)
            {
                return -1;
            }
            else
